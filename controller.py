from tkinter import Button,Label,Canvas,messagebox
...
def simulation_canvas  (parent,**config):
    global the_canvas
    parent.bind("<Key>", lambda event: model.key_pressed(event.char))
...
def LoseMessage():
    global close
    MsgBox = messagebox.askquestion ('You Lose!','Do you want to restart the application?',icon = 'warning')
    if MsgBox == 'yes':
